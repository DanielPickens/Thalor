...
func NewTerraform(workingDir string, execPath string) (*Terraform, error) {
	...
	if execPath == "" {
		err := fmt.Errorf("NewTerraform: please supply the path to a Terraform executable using execPath, e.g. using the github.com/danielpickens/hc-install module.")
...
// SetLogger specifies a logger for texec to use.
