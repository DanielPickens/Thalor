...
func TestUnparsedError(t *testing.T) {
	...
	runTest(t, "", func(t *testing.T, tfv *version.Version, tf *texec.Terraform) {
...
func TestMissingVar(t *testing.T) {
	runTest(t, "var", func(t *testing.T, tfv *version.Version, tf *texec.Terraform) {
		err := tf.Init(context.Background())
		if err != nil {
		...
		_, err = tf.Plan(context.Background(), texec.Var(longVarName+"=foo"))
		if err == nil {
			t.Fatalf("expected error running Plan, none returned")
		}

		_, err = tf.Plan(context.Background(), texec.Var(shortVarName+"=foo"))
		if err == nil {
			t.Fatalf("expected error running Plan, none returned")
		}
		...
		_, err = tf.Plan(context.Background(), texec.Var(shortVarName+"=foo"), texec.Var(longVarName+"=foo"))
		if err != nil {
...
func TestTFVersionMismatch(t *testing.T) {
	runTest(t, "tf99", func(t *testing.T, tfv *version.Version, tf *texec.Terraform) {
...
func TestLockedState(t *testing.T) {
	runTest(t, "inmem_backend_locked", func(t *testing.T, tfv *version.Version, tf *texec.Terraform) {
		err := tf.Init(context.Background())
		if err != nil {
			t.Fatalf("err during init: %s", err)
		}
...
func TestContext_sleepNoCancellation(t *testing.T) {
	...
	runTest(t, "sleep", func(t *testing.T, tfv *version.Version, tf *texec.Terraform) {
		...
		start := time.Now()
		err = tf.Apply(ctx, texec.Var(`create_duration=5s`))
		if err != nil {
