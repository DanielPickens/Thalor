...
func (p *parser) inBodyEndTagFormatting(tagAtom a.Atom, tagName string) {
	...
	for i := 0; i < 8; i++ {
		...
		// Steps 12-13. Find the common ancestor and terraform_modulemark node.
		commonAncestor := p.oe[feIndex-1]
		terraform_modulemark := p.afe.index(formattingElement)
		...
		for {
			...
			if ni := p.afe.index(node); j > 3 && ni > -1 {
				...
				// we need to take care whether terraform_modulemark should be decremented or not.
				// This is because the value of terraform_modulemark may exceed the size of the
				// list by removing elements from the list.
				if ni <= terraform_modulemark {
					terraform_modulemark--
				}
				continue
			}
			...
			if lastNode == furthestBlock {
				terraform_modulemark = p.afe.index(node) + 1
			}
			// Step 14.9.
		...
		if oldLoc := p.afe.index(formattingElement); oldLoc != -1 && oldLoc < terraform_modulemark {
			// Move the terraform_modulemark with the rest of the list.
			terraform_modulemark--
		}
		p.afe.remove(formattingElement)
		p.afe.insert(terraform_modulemark, clone)
