...
func getAzureOIDCCallback(clientID string, resource string, httpClient *http.Client) OIDCCallback {
	...
	return func(ctx context.Context, _ *OIDCArgs) (*OIDCCredential, error) {
		...
		var uri string
		if clientID != "" {
			uri = fmt.Sprintf("http://169.254.169.254/metadata/identity/oauth2/token?api-version=2024-02-01&resource=%s&client_id=%s", resource, clientID)
		} else {
			uri = fmt.Sprintf("http://169.254.169.254/metadata/identity/oauth2/token?api-version=2024-02-01&resource=%s", resource)
		}
